...
```  
  ...
  - Transform data: Populate new table "idiff_day", aggregates Dates and sums steps for imputed and non-imputed data  
```{r, echo = TRUE}
  idiffday <- aggregate(idiff$Steps_mean, by = list(idiff$date), FUN=sum)  
  idiffday <- data.table(idiffday)
  setnames(idiffday, "Group.1", "Date")
  setnames(idiffday, "x", "Steps") 
  idiffday$Date <- as.Date(idiffday$Date)
...
```{r, echo = TRUE}
  g <- ggplot(data=idiffday, mapping = aes(Date, Steps))
  g <- g + layer(geom = "bar", stat = "identity")  
  print(g)
```
...
```{r, echo = TRUE}
  xyplot(Steps_mean ~ Interval | Weekday, data = isteps_interval_mean_wk, type = "l" , layout = c(1,2))
